...
<form action='Staffreg.php' method='post'>
	<br><H2><center>Staff Registrations</center></H2><hr><br>
	<H3>
	...
	if(isset($_POST['submit'])){
		...
		if($row == 0)
		{
			if($password == $Cpassword && $fname != NULL && $lname != NULL){
				...
				VALUES (seq_id.nextval,'$username' ,'$password','$fname','$lname','$bd','$addr', 'S')";
				$parseRequest = oci_parse($conn, $query);
				oci_execute($parseRequest);
				$query2 = "INSERT INTO DB_STAFF(S_ID, ROLE, SALARY, EXP) 
				VALUES (seq_id.currval, 'none','0','none')";
				$parseRequest2 = oci_parse($conn, $query2);
				oci_execute($parseRequest2);
				$query3 = "INSERT INTO DB_STAFF_HIS_LOG(S_ID, HISTORY_LOG) 
